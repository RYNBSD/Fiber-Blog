...
type User struct {
	Id        string    `sql:"VARCHAR(36) PRIMARY KEY" json:"id"`
...
type BlogImages struct {
	...
	BlogId    string    `sql:"VARCHAR(36) NOT NULL REFERENCES blog(id) ON DELETE CASCADE" json:"blogId"`
	CreatedAt time.Time `sql:"DATETIME NOT NULL DEFAULT NOW()" json:"createdAt"`
}
...
type BlogLikes struct {
	Id        int       `sql:"BIGINT PRIMARY KEY AUTO_INCREMENT" json:"id"`
	BlogId    string    `sql:"VARCHAR(36) NOT NULL REFERENCES blog(id) ON DELETE CASCADE" json:"blogId"`
	LikerId   string    `sql:"VARCHAR(36) NOT NULL REFERENCES user(id) ON DELETE CASCADE" json:"likerId"`
	CreatedAt time.Time `sql:"DATETIME NOT NULL DEFAULT NOW()" json:"createdAt"`
}
...
type BlogComments struct {
	...
	BlogId      string    `sql:"VARCHAR(36) NOT NULL REFERENCES blog(id) ON DELETE CASCADE" json:"blogId"`
	CommenterId string    `sql:"VARCHAR(36) NOT NULL REFERENCES user(id) ON DELETE CASCADE" json:"commenterId"`
	CreatedAt   time.Time `sql:"DATETIME NOT NULL DEFAULT NOW()" json:"createdAt"`
	UpdatedAt   time.Time `sql:"DATETIME NOT NULL DEFAULT NOW() ON UPDATE NOW()" json:"updatedAt"`
}
